...
class Model():
    ...
    def show_prediction(self, skeletons, output, label, name, label_dict, prediction_savepath, video_path):
        ...
        for i in range(skeletons.shape[1]):
            ...
            for j in range(skeletons.shape[0]):
                ...
                for k in range(skeletons.shape[2]):
                    if (int(skeletons[j][i][k][0]),int(skeletons[j][i][k][1])) == (0,0):
                        continue
                    cv2.circle(img,(int(skeletons[j][i][k][0]),int(skeletons[j][i][k][1])), 5, color, -1)
